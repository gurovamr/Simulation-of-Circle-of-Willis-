...
# -------------------------------------------------------------
# User configuration (only change these 3 for a new patient/run)
# -------------------------------------------------------------
MODEL_NAME  = "cow_runV10"                   # folder name in ../models
INPUT_DIR   = "../data_patient025"          # raw JSON dir for this patient
HEART_INLET_NODE_ID = 15                    # integer node id for BA inlet, 15 -> "N15"
...
print(f"[OK] V10: 1D network has {len(arteries)} arteries and {len(unique_nodes_1d)} nodes.")
...
print(f"[INFO] V10: raw terminal nodes = {terminal_nodes}")
...
if HEART_INLET not in terminal_nodes:
    print(f"[WARN] V10: {HEART_INLET} is not degree-1, but will still be used as heart inlet.")

outlet_nodes = [n for n in terminal_nodes if n != HEART_INLET]
print(f"[INFO] V10: using {HEART_INLET} as heart inlet.")
print(f"[INFO] V10: using {len(outlet_nodes)} outlet nodes for Windkessels:")
print("       ", outlet_nodes)
...
print(f"[OK] V10: arterial.csv written -> {ARTERIAL_CSV}")
...
# MOC line:
#  - Heart inlet: (HEART_INLET, HEART_INLET)
#  - Each outlet: (Nout, Nout)
...
# no duplicate extra N15 line here

with open(MAIN_CSV, "w", newline="") as f:
    writer = csv.writer(f)
    writer.writerows(lines)

print(f"[OK] V10: main.csv written -> {MAIN_CSV}")
...
print(f"[OK] V10: wrote {len(outlet_nodes)} Windkessel p-files in {OUTPUT_DIR}")
...
if os.path.exists(ABEL_HEART):
    ...
    print(f"[OK] V10: copied heart model from {ABEL_HEART} to {COW_HEART}")
else:
    print(f"[WARN] V10: could not find {ABEL_HEART}, heart file not copied.")
