...
bool first_blood::load_main_csv()
{
    ...
    if(file_in.is_open())
    {
        ...
        while(getline(file_in,line))
        {
            ...
            else if(sv[0] == "material")
            {
                if(sv[1] == "linear") material_type = 0;
                else material_type = 1;
            }
            else if(sv[0] == "solver")
            {
                if(sv[1] == "maccormack" || sv[1] == "Maccormack" || sv[1] == "MacCormack") solver_type = 0;
                else if(sv[1] == "moc") solver_type = 1;
                else solver_type = 0;
            }
            else if(sv[0] == "moc")
            {
